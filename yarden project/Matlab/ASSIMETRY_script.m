...
for condition=1:length(conditions) %condition
    for time=1:length(Times) %before or after pas
        for subject=1:length(subjects_unique) %subjects
            ...
            POWER_Y(condition,time,subject,:) = nanmean(squeeze(10*log10(abs(F4))), 2)-nanmean(squeeze(10*log10(abs(F3))), 2); 
        end
    end
end
...
figure('WindowState','maximized');sgtitle(["Asymmetry (F4-F3); Power at Alpha band (" + num2str(freqs_RANGE_POWER(1)) + "-" + num2str(freqs_RANGE_POWER(2)) + " Hz); Phase differences at (13-16 Hz)" ;" "]);
subs=1:24;
condition_nums=1:9;
...
ylabel("Indused change in POWER Asymmetry [dB]");
...
ylabel("Indused change in POWER on Asymmetry [dB]");
...
for con=1:4
    ...
    title( ["individual average phase-delay to the PAS induced changes in power" ; "IPI = " + num2str(2*2^con) + "ms    ;  r= " + num2str(mdl.Rsquared.Ordinary.^0.5) + "; P= " + num2str(mdl.Coefficients.pValue(2))]);
    legend('off')
end
sgtitle(["Asymmetry (F4-F3); Power at Alpha band (" + num2str(freqs_RANGE_POWER(1)) + "-" + num2str(freqs_RANGE_POWER(2)) + " Hz); Phase differences at (13-16 Hz)"; " "])
