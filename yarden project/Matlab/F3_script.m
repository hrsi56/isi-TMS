...
for condition=1:length(conditions) %condition
    for time=1:length(Times) %before or after pas
        for subject=1:length(subjects_unique) %subjects
            ...
            POWER_Y(condition,time,subject,:) = nanmean(squeeze(10*log10(abs(F3))), 2); 
        end
    end
end
...
ylabel("Indused change in POWER on F3 [dB]");
...
sgtitle(["Left stimulation area (F3 electrode); Power at Alpha band (" + num2str(freqs_RANGE_POWER(1)) + "-" + num2str(freqs_RANGE_POWER(2)) + " Hz); Phase differences at (13-16 Hz) " ;" "] ,fontsize=28);
...
sgtitle(["Left stimulation area (F3 electrode); Power at Alpha band (" + num2str(freqs_RANGE_POWER(1)) + "-" + num2str(freqs_RANGE_POWER(2)) + " Hz); Phase differences at (13-16 Hz)"; " "])

%%
figure()
histogram(POWER_Y_indused_alpha_sub_eff_max_val,4)
